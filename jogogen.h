...
* \details primeiras instruções do programa Quizmaster 3001
...
void jogogen(int iduser, perguntas_rand *rand_x, alunotab *alu, int indice)	//leitura das perguntas do ficheiro e inicio do jogo
{
	...
	for(i=0;i<NUMPER_RAND;i++)
	{
		...
		usados[k] = 1;	// coloca uma flag no número aleatório já utilizado(este número representa o nr da pergunta)
...
void ranking(int iduser, alunotab *alu)
{
	...
	for( i=1;i<=NA-1;i++ )
	{
		for( j=NA;j>i;j-- )  
		{
			...
			{          
			   //trocarvalores( &temp[j].jganhos, &temp[j+1].jganhos ); 
			   //trocarvalores( &temp[j].jperdidos, &temp[j+1].jperdidos );
			   basket[j]=temp[j-1];
			   temp[j-1]=temp[j];
			   temp[j]=basket[j];
			}
	...
	for(n=NA-1;n>=NA-6;n--)
	{
		//printf("NOME=%s\t\tPerguntas certas=%d\t\tperguntas erradas=%d\t\tjogos ganhos=%d\t\tjogos perdidos=%d\n\n",
		
		jogostotal = temp[n].jganhos+temp[n].jperdidos;
		classitotal = ((temp[n].jperdidos)*10)/(jogostotal);
